...
public class InstanceInfoFactoryTests {
    ...
    public void instanceIdIsHostNameByDefault() {
        assertEquals(EurekaInstanceConfigBean.getFirstNonLoopbackAddress().getHostName(), setupInstance().getId());
    }

    @Test
    public void instanceIdIsIpWhenIpPreferred() throws Exception {
        assertTrue(setupInstance("eureka.instance.preferIpAddress:true")
                .getId().matches("(\\d+\\.){3}\\d+"));
    }
