...
public class MetricsHandlerInterceptorIntegrationTests extends AbstractMetricsTests {
	...
	public void autoConfigurationWiresTheMetricsInterceptor() {
		assertFalse(webAppContext.getBeansOfType(MetricsHandlerInterceptor.class).isEmpty());
	}
	...
	protected void assertTimer(String uriTag, String exceptionType, Integer status) {
        BasicTimer timer = ServoMonitorCache.getTimer("metricName", "method", "GET", "uri", uriTag, "caller", "unknown", "exceptionType",
                exceptionType, "status", status.toString());
