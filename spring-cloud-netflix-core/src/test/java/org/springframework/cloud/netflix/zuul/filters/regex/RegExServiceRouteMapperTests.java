...
public class RegExServiceRouteMapperTests {
    ...
    /**
     * Service pattern that follow convention {domain}-{name}-{version}. The name is optional
     */
    ...
    public void test_route_should_be_cleaned_before_returned() {
        //Messy patterns
        RegExServiceRouteMapper toTest = new RegExServiceRouteMapper(SERVICE_PATTERN+"(?<nevermatch>.)?",
                "/${version}/${nevermatch}/${domain}/${name}/");
        assertEquals("No matches for this service id", "v1/domain/service", toTest.apply("domain-service-v1"));
        assertEquals("No matches for this service id", "v1/domain", toTest.apply("domain-v1"));
    }
}
