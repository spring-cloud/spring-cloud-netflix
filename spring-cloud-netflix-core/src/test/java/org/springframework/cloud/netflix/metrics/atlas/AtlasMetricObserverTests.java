...
public class AtlasMetricObserverTests {
	...
	public void assignTypesToMetrics() {
		...
		assertHasAtlasType("rate", new Metric(new MonitorConfig.Builder("foo").build(), 0, "bar"));

		// already has type
		Metric m = new Metric(new MonitorConfig.Builder("foo").withTag(KEY, COUNTER.name())
				.withTag("atlas.dstype", "counter").build(), 0, "bar");
	...
	private void assertHasAtlasType(String atlasType, Metric m) {
		assertEquals(atlasType, AtlasMetricObserver.addTypeTagsAsNecessary(Collections.singletonList(m)).get(0)
				.getConfig().getTags().getValue("atlas.dstype"));
	}

	private Metric metricWithType(String key, DataSourceType type) {
		return new Metric(new MonitorConfig.Builder(key).withTag(KEY, type.name()).build(), 0, 1);
	}
	...
	public void metricsSentInBatches() {
		...
		mockServer = MockRestServiceServer.createServer(restTemplate);
		expectTotalBatches(mockServer, 0);
		obs.update(Collections.singletonList(new Metric(new MonitorConfig.Builder("foo").build(), 0, "nonumber")));
		mockServer.verify();
	}
	...
	private void expectTotalBatches(MockRestServiceServer mockServer, int totalBatchesExpected) {
		for (int i = 0; i < totalBatchesExpected; i++) {
			mockServer
					.expect(MockRestRequestMatchers.requestTo("atlas/api/v1/publish"))
					.andExpect(MockRestRequestMatchers.method(HttpMethod.POST))
					.andRespond(
							MockRestResponseCreators.withSuccess("{\"status\" : \"OK\"}", MediaType.APPLICATION_JSON));
		}
	}
}
