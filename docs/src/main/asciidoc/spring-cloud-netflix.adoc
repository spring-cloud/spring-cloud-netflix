...
In order to enable the warmup time, Set `eureka.server.defaultOpenForTrafficCount=0`,It enables the warmup time, and the load testing app never gets its local cache emptied once the server comes back online.But expectedNumberOfRenewsThreshold will always be at 0 and never get updated for a standalone server.Also, Netflix Eureka's waitTimeInMsWhenSyncEmpty setting is not taken into account in Spring Cloud Eureka server at the beginning.
