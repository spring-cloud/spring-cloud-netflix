...
=== Feign Health Indicator

`@FeignClient` also has `health` attribute for registering `HealthIndicator` automatically.
We can set the value of `health` with method name in feign class.
This feature only active if we include `actuator` module in our spring project.

.Example @FeignClient with health indicator
[source,java]
----
@FeignClient(name = "fooClient", health = "healthMethod")
public interface FooClient {

    @RequestMapping(method = RequestMethod.GET, value = "/health")
    ResponseEntity<String> healthMethod();
