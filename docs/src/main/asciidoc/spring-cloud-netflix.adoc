...
Zuul advance route matching can limit routing to specific http method(s) or it can route to different
endpoints for different methods on the same path. Not providing any method defaults
to match as true for any http method.
...
----
This will route any http method request on `/myusers/**` to `http://user-service`.
...
----
 zuul:
  routes:
    users:
      path: /myusers/**
      url: http://user-default-service
      routeOptions:
        allowedMethods: PUT, POST, GET
        routeConditions:
        - id: user-get-operation
          url: http://user-get-service
          predicates:
          - Method=GET
        - id: user-post-operation
          url: http://user-post-service
          predicates:
          - Method=POST
----

This will route `PUT` http method requests on `/myusers/**` to `http://user-default-service`,
`POST` http method request on `/myusers/**` to `http://user-post-service` and `GET` http
method request on `/myusers/**` to `http://user-get-service`.

Any http method other than `PUT`, `POST` and `GET` will result in 404 since that is not in the
list of allowed methods.

Advance routing configuration can be written in either compact or expanded version.
...
----
 zuul:
  routes:
    advance-routing-compact:
      path: /advanceroutescompact/**
      url: http://user-default-service
      routeOptions:
        allowedMethods: PUT, POST, GET
        routeConditions:
        - user-get-operation=http://user-get-service,Method=GET
        - user-post-operation=http://user-post-service,Method=POST
----
...
----
 zuul:
  routes:
    advance-routing-expanded:
      path: /advanceroutesexpanded/**
      url: http://user-default-service
      routeOptions:
        allowedMethods: PUT, POST, GET
        routeConditions:
