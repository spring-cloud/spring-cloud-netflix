...
HTTP method based routing is also supported. By default, if you don't specify any method
for a certain route, all the following HTTP methods will be used: GET, HEAD, POST, PUT,
PATCH, DELETE, OPTIONS, TRACE

Specifying the method(s) for a route looks like the following:
...
zuul:
  routes:
    users-cmd:
      path: /users/**
      serviceId: users-command
      methods:
        - POST
        - PUT
    users-query:
      path: /users/**
      serviceId: users-query
      methods:
        - GET
----

If any of the routes collide with each other, a warning message will be written to the logs.
