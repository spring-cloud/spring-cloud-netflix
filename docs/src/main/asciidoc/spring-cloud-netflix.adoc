...
a valid CSRF token be sent with every request to the app.  Since Eureka clients will not
generate a valid cross site request forgery (CSRF) token you will need to disable this requirement for the `/eureka/**` endpoints.
You can do this by creating a `WebSecurityConfigurerAdapter`.
