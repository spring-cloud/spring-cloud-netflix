...
|eureka.client.rest-template-timeout.idle-timeout | `+++1800000+++` | Indicates how long a connection can be idle before it is evicted from the connection pool.
...
|eureka.instance.secure-port-enabled |  | Indicates whether the secure port should be enabled for traffic or not.
